...
public abstract class PeerConnectionStatusAdapter implements
		PeerConnectionStatusListener {

	public void peerConnectionStatusChanged(PeerConnectionStatusEvent event) {
		PeerConnection.Status status = event.getStatus();
		PeerConnection connection = event.getConnection();
		
		if (status.equals(PeerConnection.Status.OPENED)) {
			connectionOpened(connection);
		} else if (status.equals(PeerConnection.Status.CLOSED)) {
			connectionClosed(connection);
		} else if (status.equals(PeerConnection.Status.ACTIVATED)) {
			connectionActivated(connection);
		}
	}
