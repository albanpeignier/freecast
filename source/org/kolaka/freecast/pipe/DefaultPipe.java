...
import org.kolaka.freecast.packet.*;
...
public class DefaultPipe implements Pipe {
    ...
    class DefaultConsumer extends BaseConsumer implements Cloneable {
        ...
        public synchronized void push(Packet packet) {
            Long sequenceNumber = new Long(packet.getElementDescriptor().getPageDescriptor().getSequenceNumber());
			LogicalPageBuilder builder = (LogicalPageBuilder) pageBuilders.get(sequenceNumber);
            
            if (builder == null) {
                builder = new TimedLogicalPageBuilder(packet.getElementDescriptor().getPageDescriptor());
                pageBuilders.put(sequenceNumber, builder);
            }

			if (this != nextConsumer) {
				// LogFactory.getLog(getClass()).debug("push packet: " + sequenceNumber + "/" + packet.getSequenceNumber() + " - " + pageBuilders.size());
			}
            builder.add(packet);

            if (pageBuilders.size() > (queueLength * 1.2)) {
                int initialBuilderCount = pageBuilders.size(); 
                for (Iterator iter = pageBuilders.values().iterator(); pageBuilders.size() > queueLength && iter.hasNext();) {
                    ...
                    if (removedBuilder.isComplete()) {
						...
						LogFactory.getLog(getClass()).debug("remove incomplete builder " + removedBuilder);
					}
                }
                if (this != nextConsumer) {
                    String message = "removed in " + name + " " + (initialBuilderCount - pageBuilders.size()) + " builders";
        ...
        public synchronized LogicalPage consume() throws EmptyPipeException {
            if (headerPage != null) {
                LogFactory.getLog(getClass()).debug("Use cached first page: " + headerPage);
            ...
            do {
                ...
                if (!builder.isComplete() && !iterator.hasNext() && builder.getLiveTimeLength() < 30000) {
                    LogFactory.getLog(getClass()).debug("single page incomplete kept: " + builder);
                    throw new EmptyPipeException();
                }
                ...
                if (!builder.isComplete()) {
                    LogFactory.getLog(getClass()).debug("incomplete page skipped: " + builder);
                    builder = null;
