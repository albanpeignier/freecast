...
public abstract class BaseConfigurationLoader implements ConfigurationLoader {
    ...
    protected void logConfiguration() {
        ...
        LogFactory.getLog(getClass()).debug("loaded configuration: " + sb.toString());
    }

    protected Configuration loadUserConfiguration() throws ConfigurationException {
        if (userURI == null) {
            return new BaseConfiguration();
        }
    
        LogFactory.getLog(getClass()).debug("load the user configuration from " + userURI);
        XMLConfiguration configuration = new XMLConfiguration();
		try {
			configuration.load(locator.openResource(userURI));
		} catch (ResourceLocator.Exception e) {
			throw new ConfigurationException("Can't load the user configuration URI " + userURI,e);
		}
    ...
    protected Configuration loadDefaultConfiguration(String name) throws ConfigurationException {
        URL url = getClass().getResource("resources/config-" + name + ".xml");
        if (url == null) {
            throw new ConfigurationException("Can't find the default configuration settings:" + name);
        }
        LogFactory.getLog(getClass()).debug("load the default configuration from " + url);
        ConfigurationFactory factory = new ConfigurationFactory();
        factory.setConfigurationURL(url);
        Configuration configuration = factory.getConfiguration();
        if (configuration.isEmpty()) {
            throw new ConfigurationException("No default configuration found for " + url);
        }
        return configuration;
    }
    ...
    public void load() throws ConfigurationException {
        if (configuration != null) {
            LogFactory.getLog(getClass()).warn("configuration reloading not supported");
            return;
        }
    ...
    protected Configuration loadConfiguration() throws ConfigurationException {
        ...
        if (!userProperties.isEmpty()) {
            configuration.addConfiguration(new MapConfiguration(userProperties));
        }
    ...
    protected abstract void completeConfiguration(CompositeConfiguration configuration) 
