...
public class TimedOggSource implements OggSource {
	...
	private long initialAbsoluteGranulePosition = -1;
	private final OggSource source;
	private final float millisPerSample;
	
	public TimedOggSource(OggSource source, int sampleRate) {
		this.source = source;
		this.millisPerSample = sampleRate / (float) DateUtils.MILLIS_PER_SECOND;
	}
	...
	private long getTimestamp(OggPage sourcePage) {
		...
		if (initialAbsoluteGranulePosition == -1) {
		...
		return (long) (relativePosition / millisPerSample);
	}
