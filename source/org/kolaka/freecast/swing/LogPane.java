...
public class LogPane extends JPanel {
    ...
    public LogPane() {
        ...
        table.getColumnModel().getColumn(0).setCellRenderer(new LevelCellRenderer());
    ...
    class LevelCellRenderer extends DefaultTableCellRenderer {
        ...
        private final Icon DEFAULT_ICON = new ImageIcon(getClass().getResource("resources/log.png"));
        
        private void load(Level level, String resource) {
            icons.put(level, 
                    new ImageIcon(getClass().getResource(resource)));
        }
    ...
    class SaveAction extends BaseAction {
        ...
        public void actionPerformed(ActionEvent event) {
            ...
            save(file);
            JOptionPane.showMessageDialog(LogPane.this, "FreeCast log has been into\n" + file.getAbsolutePath());
        }
    ...
    class EmailAction extends BaseAction {
        ...
        public void actionPerformed(ActionEvent event) {
            try {
                ...
                message.setToAddrs(Collections.singletonList("freecast-support@lists.tryphon.org"));
                message.setSubject("FreeCast log report");
                message.setBody("FreeCast network:\nProblem description:\n");

                message.setAttachments(Collections.singletonList(file.getAbsolutePath()));

                Desktop.mail(message);
            } catch (Exception e) {
                LogFactory.getLog(getClass()).error("can't prepare a log email", e);
            }
        }
