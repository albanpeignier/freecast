...
(JNIEnv* env, jobject obj, jobjectArray afValues, jint nValues)
{
	...
	handle = getHandle(env, obj);
	bufferRoot = vorbis_analysis_buffer(handle, nValues);
	if (debug_flag) { fprintf(debug_file, "Java_org_tritonus_lowlevel_vorbis_DspState_write(): bufferRoot: %p\n", bufferRoot); }
	if (afValues != NULL)
	{
